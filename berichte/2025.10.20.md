...
Nmap ist ein Kommandozeilen‑Tool zum Auffinden von Hosts und zum Scannen von Netzwerkdiensten. Ein Ping‑Scan in Nmap wird mit der Option `-sn` ausgeführt und ermittelt nur welche Hosts im Zielnetz „up“ sind, ohne Ports zu scannen. Als Root im lokalen LAN nutzt es ARP‑Requests (sehr zuverlässig), die von Firewalls geblockt werden können. Die Ausgabe meldet für gefundene Geräte typischerweise „Host is up“ und liefert bei ARP‑Erkennung zusätzlich MAC‑Adressen und Vendor‑Infos.

### Recherche zu Erweiterungen von Nmap

-sS — Führt einen halboffener Scan durch, der schnell Ports erkundet, ohne vollständige Verbindungen aufzubauen.
...
-A — Aktiviert aggressive Erkennung für umfassende Informationen.
...
- Im Router oder Switch nachschauen     
  Man öffnet die Verwaltungsseite oder tippt im Gerät den Befehl für die angeschlossenen Geräte und sieht direkt, welche IP-Adressen und Geräte im Netz sind.

- Passiv zuhören im Netz      
  Man lässt den eigenen Rechner mitlaufen und hört mit, ob andere Geräte im Netzwerk kleine Nachrichten senden. So tauchen die Geräte auf, ohne dass man sie extra anfragt.

- Kurz Dienste anfragen      
  Man schickt statt Ping kleine Verbindungsanfragen an bekannte Dienste. Wenn etwas antwortet, ist das Gerät da — auch wenn Ping blockiert ist.
...
- ICMP‑Ping blockieren oder nur gezielt erlauben.    
...
- Antwort‑Rate für Pings begrenzen (Rate‑Limit).     
...
- Stateful‑Firewall einsetzen, die nur legitime Antworten durchlässt.    
